...
export function v1ListsRetrieveQueryOptions<
  TData = V1ListsRetrieve["response"],
>(
  options: V1ListsRetrieve["client"]["parameters"] = {},
): SWRConfiguration<TData, V1ListsRetrieve["error"]> {
  return {
    fetcher: async () => {
 ...
 * @link /api/v1/lists
 */
export function useV1ListsRetrieve<
  TData = V1ListsRetrieve["response"],
>(options?: {
  ...
  const query = useSWR<TData, V1ListsRetrieve["error"], typeof url | null>(
