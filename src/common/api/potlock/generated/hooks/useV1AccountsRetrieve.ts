...
export function v1AccountsRetrieveQueryOptions<
  TData = V1AccountsRetrieve["response"],
>(
  options: V1AccountsRetrieve["client"]["parameters"] = {},
): SWRConfiguration<TData, V1AccountsRetrieve["error"]> {
  return {
    fetcher: async () => {
 ...
 */
export function useV1AccountsRetrieve<
  TData = V1AccountsRetrieve["response"],
>(options?: {
  ...
  const query = useSWR<TData, V1AccountsRetrieve["error"], typeof url | null>(
