...
export function v1AccountsActivePotsRetrieveQueryOptions<
  TData = V1AccountsActivePotsRetrieve["response"],
>(
  accountId: V1AccountsActivePotsRetrievePathParams["account_id"],
  params?: V1AccountsActivePotsRetrieve["queryParams"],
  options: V1AccountsActivePotsRetrieve["client"]["parameters"] = {},
): SWRConfiguration<TData, V1AccountsActivePotsRetrieve["error"]> {
  return {
    fetcher: async () => {
 ...
 */
export function useV1AccountsActivePotsRetrieve<
  TData = V1AccountsActivePotsRetrieve["response"],
>(
  accountId: V1AccountsActivePotsRetrievePathParams["account_id"],
  params?: V1AccountsActivePotsRetrieve["queryParams"],
  ...
  const query = useSWR<
    TData,
    V1AccountsActivePotsRetrieve["error"],
    [typeof url, typeof params] | null
  >(shouldFetch ? [url, params] : null, {
    ...v1AccountsActivePotsRetrieveQueryOptions<TData>(
      accountId,
      params,
      clientOptions,
    ),
