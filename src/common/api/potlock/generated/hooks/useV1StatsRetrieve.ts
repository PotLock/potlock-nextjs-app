...
export function v1StatsRetrieveQueryOptions<
  TData = V1StatsRetrieve["response"],
>(
  options: V1StatsRetrieve["client"]["parameters"] = {},
): SWRConfiguration<TData, V1StatsRetrieve["error"]> {
  return {
    fetcher: async () => {
 ...
 * @link /api/v1/stats
 */
export function useV1StatsRetrieve<
  TData = V1StatsRetrieve["response"],
>(options?: {
  ...
  const query = useSWR<TData, V1StatsRetrieve["error"], typeof url | null>(
