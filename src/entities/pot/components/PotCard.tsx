...
/**
 * @deprecated Use {@link useFungibleToken} capabilities.
 */
export const useMatchingPoolBalance = ({ pot }: { pot?: Pot }) => {
  const { data: nearToken } = useFungibleToken({ tokenId: NATIVE_TOKEN_ID });
...
export const PotCard: React.FC<PotCardProps> = ({ potId }) => {
  ...
  return !pot ? (
    // Card
    <div className="bg-background flex h-full min-h-[300px] w-full flex-col items-center justify-center rounded-[8px] pb-1.5 shadow-[inset_0px_-2px_0px_0px_#464646,0px_0px_0px_1px_#464646] hover:cursor-pointer">
    ...
    <Link
      ...
      className="bg-background flex h-full min-h-[300px] w-full flex-col rounded-[8px] pb-1.5 shadow-[inset_0px_-2px_0px_0px_#464646,0px_0px_0px_1px_#464646] hover:cursor-pointer hover:no-underline"
    >
      {/* Card Section */}
